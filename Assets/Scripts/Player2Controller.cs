...
public class Player2Controller : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (collision.collider.CompareTag("Rocket2") && !hasScored)
        {
            hasScored = true;
            AdvanceLevel();
        }
    ...
    void AdvanceLevel()
    {
        ...
        {
            StartCoroutine(ShowlevelUpImage2());
            FindObjectOfType<GameController>().PlayerScored(2);  
            Respawn();
            hasScored = false;
        }
        else
    ...
    IEnumerator ShowlevelUpImage2()
    {
        levelUpImage2.gameObject.SetActive(true);
        yield return new WaitForSeconds(1);
        levelUpImage2.gameObject.SetActive(false);
    }
    ...
    void Respawn()
    {
        ...
        {
            Vector2 respawnPosition = FindObjectOfType<GameController>().astronautPositions2[currentLevel - 1];  // o astronautPositions2 para Player2Controller
        ...
        else
        {
            ...
            Vector2 respawnPosition = FindObjectOfType<GameController>().astronautPositions2[9];  // o astronautPositions2 para Player2Controller
            transform.localPosition = new Vector3(respawnPosition.x, respawnPosition.y, startPosition.z);
            rb.velocity = Vector2.zero;
        }
    }
